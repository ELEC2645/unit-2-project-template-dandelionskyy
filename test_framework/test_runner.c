...
int verify_test_result(TestCase* test_case, Table* result_table) 
{
    ...
    // Validate row count
    if (test_case->expected_row_count >= 0 && result_table->row_count != test_case->expected_row_count) 
    {
    ...
    if (test_case->expected_column_count > 0 && result_table->col_count != test_case->expected_column_count) 
        {
